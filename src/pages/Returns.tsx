...
const Returns = () => {
  ...
  const borrowed = transactions.filter(t => t.status === 'borrowed' || t.status === 'late');
  const filtered = borrowed.filter(t => t.borrowerName.toLowerCase().includes(search.toLowerCase()) || t.bookTitle.toLowerCase().includes(search.toLowerCase()));
  ...
  const handleReturn = (id: string) => {
    ...
    setTransactions(prev => prev.map(t => {
      ...
      return {
        ...
        status: isLate ? 'late' as const : 'returned' as const,
      };
    }));
              ...
              <tbody>
                {filtered.length === 0 ? (
                  ...
                  <tr key={t.id} className="border-b last:border-0 hover:bg-muted/20 transition-colors">
                    ...
                    <td className="p-3 text-muted-foreground">{t.dueDate.split('T')[0]}</td>
                    ...
                    <td className="p-3 text-right">
                      <Button size="sm" variant="outline" onClick={() => handleReturn(t.id)}>
