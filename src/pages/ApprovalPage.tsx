...
import { Search, CheckCircle, XCircle, Clock, Filter, BookOpen, User, GraduationCap, Users } from 'lucide-react';
...
const ApprovalPage = () => {
  const { user } = useAuth();
  const { requests, updateRequestStatus } = useBorrowRequests();
  ...
  const filtered = requests.filter(r => {
    const matchSearch = r.requesterName.toLowerCase().includes(search.toLowerCase()) || r.bookTitle.toLowerCase().includes(search.toLowerCase());
  ...
  const handleApprove = (id: string) => {
    updateRequestStatus(id, 'approved', user?.name || 'Admin');
  ...
  const handleReject = () => {
    ...
    updateRequestStatus(selectedReqId, 'rejected', user?.name || 'Admin', rejectionReason || 'Ditolak oleh admin');
                    ...
                    <tr key={r.id} className="border-b last:border-0 hover:bg-muted/20 transition-colors">
                      <td className="p-3">
                        <div className="flex items-center gap-2">
                          <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 ${r.requesterRole === 'siswa' ? 'bg-primary/10' : 'bg-accent'}`}>
                            {r.requesterRole === 'siswa' ? <GraduationCap className="w-3.5 h-3.5 text-primary" /> : <Users className="w-3.5 h-3.5 text-foreground" />}
                          </div>
                          <div>
                            <p className="font-medium text-foreground text-sm">{r.requesterName}</p>
                            <p className="text-[10px] text-muted-foreground capitalize">
                              {r.requesterRole}{r.className ? ` • ${r.className}` : ''}
                              {r.duration ? ` • ${r.duration} hari` : ''}
                            </p>
                          </div>
                        </div>
                      </td>
                      <td className="p-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-foreground">{r.bookTitle}</span>
                        </div>
                      </td>
                      ...
                      <td className="p-3 text-right">
                        {r.status === 'pending' ? (
                          ...
                          <span className="text-xs text-muted-foreground">
                            {r.reviewedBy && `oleh ${r.reviewedBy}`}
                          </span>
                        )}
                      </td>
                    </tr>
